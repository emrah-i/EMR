...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-[100] h-[82px] border-b border-emr-light-gray/65 bg-white/95 backdrop-blur-[14px] max-[960px]:h-[72px]">
      <div className="relative mx-auto flex h-full w-full max-w-[1160px] items-center justify-between px-6 max-[760px]:px-4">
        <a className="cursor-pointer" href="#top" aria-label={`${siteConfig.companyName} home`} onClick={closeMenu}>
          <img className="h-auto w-[132px] max-[960px]:w-28" src="/emr-commerce-logo.png" alt="EMR Commerce" />
        </a>

        <button
          ref={menuButtonRef}
          className="hidden h-11 w-11 cursor-pointer rounded-md border border-emr-border bg-white px-3 max-[760px]:block"
          type="button"
        ...
        >
          ...
          <a
            className="inline-flex min-h-11 cursor-pointer items-center justify-center rounded-md border border-emr-navy bg-emr-navy px-[18px] py-[11px] text-[0.85rem] font-bold leading-[1.2] text-white transition duration-200 hover:-translate-y-px hover:bg-emr-navy-light motion-reduce:transform-none max-[760px]:mt-2"
            href="#contact"
            onClick={closeMenu}
          >
            Get in Touch
